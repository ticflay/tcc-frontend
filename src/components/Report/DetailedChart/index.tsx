...
export default function EnvironmentChart({
  ...
  return (
    <div className="flex flex-row gap-8 overflow-hidden">
      <div className="flex flex-col gap-7">
        <div className="flex flex-row gap-4 items-center">
          <Button
            ...
            text={t("Ver desempenho geral")}
            onClick={() => setSelectedMenu("Geral")}
          />
          <span className="text-lg font-bold">{t(selectedMenu)}</span>
        </div>
      ...
      <div className="w-full flex overflow-hidden max-w-full flex-col border border-solid flex-1  rounded-md">
        <div className="flex flex-row border-b border-solid mx-3 overflow-auto">
          {categories
            ?.filter((item) => {
              const criteriion = criteria?.find((c) => c.name === selectedMenu);
                ...
                <button
                  ...
                  className={`${
                    selectedCategory?.id === category.id
                      ? "font-bold  text-secondary-berkeley_blue border-b-2 border-solid border-b-secondary-berkeley_blue"
                      : ""
                  } text-center hover:opacity-80 text-base text-gray-600 px-4 py-2 whitespace-nowrap `}
                >
                  {t(category.name)}
                </button>
              );
        ...
        {selectedCategory && categories && (
          <div className="m-3 flex flex-col gap-5">
            <div className="flex flex-row justify-between">
              ...
              <span>
                {t("Nota da categoria")}:{" "}
                {(calculateGrade(selectedCategory, questions, formAnswers))
                  .toFixed(2)
                  .replace(".", ",")}
                /5
              </span>
